...
contract SimpleTokenSwap {
    ...
    address public WETH;
    // Setting Uniswap Router address and WETH address in constructor
    constructor(address _swapRouter, address _WETH) {
        swapRouter = ISwapRouter(_swapRouter);
        WETH = _WETH;
    }
    // Create a swap function that takes input and output token addresses,
    // input amount, minimum output amount, and recipient's address
    function swap(
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        ...
        require(IERC20(tokenIn).transferFrom(msg.sender, address(this), amountIn), "Transfer failed");
         // Approve Uniswap router to spend input tokens
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
         ...
         // Call  Uniswap router's exactInputSingle function to execute swap
